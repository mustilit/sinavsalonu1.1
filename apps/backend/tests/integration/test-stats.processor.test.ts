...
describe('TestStats processor integration', () => {
  ...
  test('processor computes aggregates and upserts TestStats', async () => {
    ...
    // purchases
    await prisma.purchase.createMany({ data: [{ testId: test.id, candidateId: candidate1.id, amountCents: 100 }, { testId: test.id, candidateId: candidate2.id, amountCents: 100 }, { testId: test.id, candidateId: candidate1.id + 'x', amountCents: 100 }] });
